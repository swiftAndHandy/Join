...
function updateBoard(updateItem, fromLocation, targetLocation) {
  ...
  document
    .getElementById(`${targetLocation}-field`)
    .insertAdjacentElement("beforeend", transfer);
  transfer.setAttribute(
    "ondragstart",
    `startDrag('${updateItem}', '${targetLocation}')`
  );
...
function setBottomTaskCardVisibility(card, taskId) {
  if (emptyBottomOf(card)) {
    document
      .getElementById(`bottom-board-card-wrapper${taskId}`)
...
function emptyBottomOf(card) {
  return (
    (!card.assigned || card.assigned.length === 0) &&
...
function updateTaskFields(sections) {
  for (let item in sections) {
    hideWindow(
      `task-field-${sections[item]}`,
      document.getElementById(`${sections[item]}-field`).innerHTML.trim() !== ""
    );
  }
}
...
function rerenderTaskOnBoard(data, taskId) {
  ...
  const hiddenDescription = document.getElementById(
    `hidden-description-content${taskId}`
  );
...
function showSearchErrors() {
  ...
  totalResults > 0
    ? hideWindow("empty-results")
    : hideWindow("empty-results", false);
...
function resultsAmountOf(field) {
  ...
  let value = document
    ...
    .querySelectorAll(":scope > :not(.d-none)").length;
  if (
    document.getElementById(`task-field-${field}`).classList.contains("d-none")
  ) {
...
function showFieldsWhenSearch() {
  ...
  sections.forEach(function (section) {
    ...
    if (
      fieldElement.classList.contains("d-none") &&
      `${section}-field` !== ""
    ) {
