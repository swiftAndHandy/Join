  ...
  try {
    ...
    const contacts =
      results[0].status === "fulfilled" && results[0].value
        ? setPrefixToKey("contacts", results[0].value)
        : {};
    const accounts =
      results[1].status === "fulfilled" && results[1].value
        ? setPrefixToKey("accounts", results[1].value)
        : {};

    const data = { ...accounts, ...contacts };

    sortByAlphabet(data).forEach(generateContactsHtml);
  } catch (error) {
...
function searchContact(search) {
  const contactNames = Array.from(
    document.querySelectorAll("div.user-box span")
  );
  for (let item in contactNames) {
    hideWindow(
      contactNames[item].parentElement.parentElement.id,
...
async function toggleThisContact(id) {
  ...
  const pseudoCheckboxId = document.getElementById(
    `assign-contact-pseudo-checkbox-${id}`
  );
...
function generateContactsHtml(data) {
  ...
  target.innerHTML += `
        <div id="assign-contact-${
          data.path
        }" class="single-contact" onclick="toggleThisContact('${data.path}')">
            <div class="user-box">
                <div id="edit-task-avatar-${
                  data.id
                }" class="avatar at-drop-down" style="background-color: ${
    data["color"]
  };">${initials(data.name)}</div>
                    <span id="edit-task-username-${data.id}">${data.name}</span>
                </div>
            <div id="assign-contact-pseudo-checkbox-${
              data.path
            }" class="pseudo-checkbox">
            </div>
                <input type="checkbox" class="contact-checkbox" id="assign-contact-checkbox-${
                  data.path
